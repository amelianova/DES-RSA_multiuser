...
def encrypt_message(plaintext_str, key_hex):
    """Enkripsi sebuah string plaintext (bukan file)."""
    if len(key_hex) != 16:
        raise ValueError("Kunci Heksadesimal harus 16 karakter")
    
    key_64bit = hex_to_bin(key_hex)
    round_keys = key_schedule(key_64bit)
    
    padded_text = pad_text(plaintext_str)
    ...
    # Mengembalikan sebagai hex agar aman dikirim lewat jaringan
    return bin_to_hex(cipher_binary)

def decrypt_message(cipher_hex_str, key_hex):
    """Dekripsi sebuah string ciphertext (bisa panjang dinamis)."""
    if len(key_hex) != 16:
        raise ValueError("Kunci Heksadesimal harus 16 karakter")
    ...
    cipher_hex_str = cipher_hex_str.strip()
    # Pastikan panjang ciphertext kelipatan 16 (1 blok = 16 hex digit)
    if len(cipher_hex_str) % 16 != 0:
        raise ValueError(f"Panjang ciphertext ({len(cipher_hex_str)}) tidak valid, harus kelipatan 16 hex digit")

    decrypted_binary = ''
    for i in range(0, len(cipher_hex_str), 16):
        block_hex = cipher_hex_str[i:i + 16]
        block_bin = hex_to_bin(block_hex)
        decrypted_binary += des_decrypt_block(block_bin, round_keys)
